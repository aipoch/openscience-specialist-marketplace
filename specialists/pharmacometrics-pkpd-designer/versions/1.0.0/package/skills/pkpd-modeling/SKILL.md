...
## Open Science regression safeguards

- Convert per-kilogram animal CL and V to total-animal quantities before total-body allometry. Keep each half-life calculation within one named CL/V scenario; crossed methods are combination sensitivities.
- Protein binding correction requires an explicit total-versus-unbound clearance model and must not be applied twice. Do not derive Vss mechanically as Vss/fu.
- MABEL is not a fixed receptor-occupancy percentage. Missing potency, target expression/turnover, TE/PD, NOAEL exposure or human F blocks an operational FIH dose.
