...
- For magnitudes use m = -2.5 log10(F/F0). As positive flux tends to zero, magnitude tends to positive infinity. Non-detections are flux upper limits and magnitude lower bounds m > m_lim.
- Never invent gain, read noise, dark-current temperature scaling, zero points, extinction coefficients, or calibrated source values when headers or calibration evidence are absent.
